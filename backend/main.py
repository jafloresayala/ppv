...
def financials_currency_rate(req: CurrencyRateRequest):
    ...
    `is_fallback=true` means SAP had no real rate for this currency/date (even
    after backfilling prior days) and 1.0 is being returned only as a
    last-resort placeholder — callers must not cache it as a genuine rate.
    """
    from_currency = (req.from_currency or "").strip().upper()
    if not from_currency or from_currency == "USD":
        return {"rate": 1.0, "from_currency": from_currency or "USD", "date": (req.date or "")[:10], "is_fallback": False}

    date_str = (req.date or "")[:10]
    if not date_str:
        raise HTTPException(status_code=400, detail="date is required")

    rate, is_fallback = get_currency_rate(from_currency, date_str)
    return {"rate": rate, "from_currency": from_currency, "date": date_str, "is_fallback": is_fallback}
